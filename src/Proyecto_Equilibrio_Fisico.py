...
# Bloque de codigo donde el usuario inserta los datos
try: 
    # Introduccion
    input('Proyecto Equilibrio Físico: Pablo Vargas, Regina Noriega => Grafica la temperatura de burbuja y temperatura de rocio de una mezcla de tres componentes en un diagrama ternario. Presiona [Enter] para continuar')

    # Obtenemos compuesto 1, corroboramos que cuente con todos los datos necesarios e imprimimos su temperatura de ebullicion normal
    compuesto_1 = input('Inserte: nombre, numero ID o codigo CAS del primer compuesto: ')
    teb, nombre_1, tc, pc, w = f.obtener_vectores_propiedades_objeto([compuesto_1], ['tbp', 'nombre', 'tc', 'pc', 'w'])
    f.comprobar_existencia_de_datos(nombre_1, pc, tc, w)
    print(f'Temperatura de ebullicion de {nombre_1[0,0]}: {round(teb[0,0] - 273.15, 1)}°C')

    # Compuesto 2
    compuesto_2 = input('Compuesto 2: ')
    teb, nombre_2, tc, pc, w = f.obtener_vectores_propiedades_objeto([compuesto_2], ['tbp', 'nombre', 'tc', 'pc', 'w'])
    f.comprobar_existencia_de_datos(nombre_2, pc, tc, w)
    print(f'Temperatura de ebullicion de {nombre_2[0,0]}: {round(teb[0,0] - 273.15, 1)}°C')

    # Compuesto 3
    compuesto_3 = input('Compuesto 3: ')
    teb, nombre_3, tc, pc, w = f.obtener_vectores_propiedades_objeto([compuesto_3], ['tbp', 'nombre', 'tc', 'pc', 'w'])
    f.comprobar_existencia_de_datos(nombre_3, pc, tc, w)
    print(f'Temperatura de ebullicion de {nombre_3[0,0]}: {round(teb[0,0] - 273.15, 1)}°C')
    ...
    # Definimos si se detiene el programa si no se encuentra una solucion
    modo_libre = input('Deseas que el programa continue ejecutandose a pesar de errores de calculo? [SI/NO]: ')
    modo_libre = f.procesar_modo_libre(modo_libre)
except Exception as error:
    print(error)
    input('Sucedio un error. Presiona [ENTER] para cerrar programa')
    raise ValueError('Cerrando')


# Generamos lista de nombres de compuestos
    ...
    for i in range(filas):
        for j in range(columnas):
            try:
                ...
                print(error)
                input('Sucedio un error. Presiona [ENTER] para cerrar el programa')
                raise ValueError('Cerrando')
...
ax.plot_surface(x, y, temperatura_burbuja, color='#007ead', alpha=0.9)
ax.plot_surface(x, y, temperatura_rocio, cmap=cm.inferno, alpha=1)
...
ax.text(x=0, y=0, z=temperatura_sat[0], s=nombre_1[0,0])
ax.text(x=0.5, y=1.5, z=temperatura_sat[1], s=nombre_2[0,0])
ax.text(x=1, y=0, z=temperatura_sat[2], s=nombre_3[0,0])
